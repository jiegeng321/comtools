    ...
    for object in objects:
        ...
        if file_name[-1] != "/":
            _local_dir = local_dir + \
                file_name.split(
                    "/")[-1] if local_dir[-1] == "/" else local_dir + "/" + file_name.split("/")[-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--action", type=str, default="download",
                        required=True, choices=["upload", "download"],
    ...
    try:
        ...
        print('failed to connect oss server.')
        print(str(e))
    else:
