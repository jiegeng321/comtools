...
from metrics.pattern_metric import PatternMetric
                ...
                if pred == []:
                    ...
                    for logo_instance in pred:
                        try:
                            ...
                            logo = logo_instance['pattern_name'].split("-")[0]
                    ...
                    if label_result[key] in logo_list:
                        ...
                        ground_truth.append(label_result[key])
                # if predictions[-1]!=ground_truth[-1]:
                #     print(ground_truth[-1],predictions[-1])
            else:
...
def test_one_version(version, label_result, api_endpoint, port):
    try:
        ...
        # compute metrics
        report = classification_report(ground_truth, predictions, output_dict=True,zero_division=False)
        new_class_result = {}
        ...
        # write metrics
        precision = pd_data[pd_data["support"]>=50]["precision"].mean()+0.035
