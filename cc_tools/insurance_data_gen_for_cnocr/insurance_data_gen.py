...
def info_gen(num):
    ...
    card_info = {}
    card_info['char_num'] = [''.join([random.choice(char_list+num_list) for i in range(20)]) for j in range(num)]
...
def data_gen(txt_info):
   # save_path = 'cnocr_gen_data'
    ...
    for i in tqdm(range(len(train_items))):
        ...
        len_total = 16*num_len + 17*char_len + 31*date_len
        ...
        if random.random() < 0.5:
            ...
            for j in range(random.randint(600,1400)):
        ...
        if random.random() < 0.5:
            ...
            target = target.resize((340, 32),resample=Image.BOX)
