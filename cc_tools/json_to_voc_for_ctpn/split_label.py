...
for file in tqdm(files):
    ...
    im_scale = float(image_resize[0]) / float(im_size_min)
    if np.round(im_scale * im_size_max) > image_resize[1]:
        im_scale = float(image_resize[1]) / float(im_size_max)
