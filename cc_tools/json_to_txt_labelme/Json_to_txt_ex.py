...
ex = 1
json_file_path = "face_pts_prelabel_comb_6parts"
txt_save_path = "face_pts_prelabel_comb_6parts_txt"
img_save_path = "face_pts_prelabel_comb_6parts_img"
...
for file in tqdm(list_path):
    ...
    if shapes!=[] and len(shapes)==4:
        if ex == 1:
            img = Image.open(os.path.join(json_file_path, file.replace(".json", ".jpg")))
            w = img.width
            img_ex = img.transpose(Image.FLIP_LEFT_RIGHT)
            img_ex.save(os.path.join(img_save_path, file.replace(".json", "_ex.jpg")))
        ...
        if ex==1:
            po = []
            with open(os.path.join(txt_save_path, os.path.splitext(file)[0] + "_ex.txt"), 'w') as f:
                for shape in shapes:
                    for point in shape["points"]:
                        for i,xy in enumerate(point):
                            if i==0:
                                new = w - xy
                            else:
                                new = xy
                            po.append(new)
                            f.write(str(new) + ' ')
        imagedraw = ImageDraw.Draw(img_ex)
        imagedraw.line(po, (255, 0, 0), width = 5)
        img_ex.save(os.path.join(img_save_path, file.replace(".json", "_ex_draw.jpg")))
