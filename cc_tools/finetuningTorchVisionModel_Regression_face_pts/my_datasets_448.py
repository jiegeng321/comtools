...
class DatasetImgTxtPair(Dataset):
    ...
    def __init__(self, root, train_val="train", transform = None, target_transform=None,size = 144):
    ...
    def __getitem__(self, index):
        ...
        if angle !=0:
            ...
            for i in range(22):
                ...
                label_rot.append(y)
        if angle !=0:
            label = [float(i)*size/wd if n % 2 ==0 else float(i)*size/ht for n,i in enumerate(label_rot)]
        else:
            label = [float(i)*size/wd if n % 2 ==0 else float(i)*size/ht for n,i in enumerate(label)]
