...
from src.pattern_detector_trt import PatternDetector
import logging
...
APPNAME = os.environ.get("APPNAME", "ai-brand-Pattern")
...
pattern_detector = PatternDetector(logger)
pattern_detector.warmup()
    ...
    def on_post(self, req, resp):
        ...
        try:
            ...
            with Timer() as algo_timer:
                res = pattern_detector.detect(image, log_recorder)
...
meta_data = MetaData()
app.add_route("/pattern_rec_base64", api_service)
app.add_route("/v2/pattern_brand_rec", api_service)
app.add_route("/ok.htm", validate_service)
app.add_route("/actuator/prometheus", Metrics_Resource)
app.add_route("/get_pattern_metadata", meta_data)
