...
import ast
txt_paths = ["/data01/xu.fx/dataset/NEW_RAW_INCREASE_DATA/FORDEAL_ONLINE_TXT_DATA/0210-0219.txt",
...
dst_dir = "/data01/xu.fx/dataset/NEW_RAW_INCREASE_DATA/clip_test"

total_brand = ['禁销宗教猪', "禁销电子类违禁产品"]
need_brand = {}
...
other_brand_num = 0
auto_result_filter = None#"Reject" or "Accept"
...
for index,line in enumerate(data):
    ...
    if "finalTagHit" not in dict_brand:continue
    finalTagHit = dict_brand["finalTagHit"]
    if not ("禁销" in finalTagHit or "违禁" in finalTagHit or "涉政" in finalTagHit):continue
    ...
    for img in img_list:
        try:
            ...
            if (auto_result == auto_result_filter or auto_result_filter == None) \
                    ...
                    and ("禁销" in finalTagHit or "违禁" in finalTagHit or "涉政" in finalTagHit):#ruleHit
                #print(img)
                url = img["textData"]
                final_brand_raw = [i.split("[图像]")[-1].split("】")[0].replace("/","") for i in list(set(img["finalTagHit"].split(",")))]
                # for j in range(len(final_brand_raw)):
                #     if "禁销电子类违禁产品" in final_brand_raw[j]:
                #         final_brand_raw[j]="禁销电子类违禁产品"
                if final_brand_raw==[]:
                    continue
                final_brand = []
                for brand in final_brand_raw:
                    # if brand in need_brand:
                    #     final_brand.append(brand)
                    # if "禁销电子类违禁产品" in brand:
                    #     final_brand.append(brand)
                    for k,v in need_brand.items():
                        if k in brand:
                            final_brand.append(brand)
                if final_brand==[]:
                    continue
                print_name = final_brand[0]
