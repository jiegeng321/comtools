...
dst_dir = "/data01/xu.fx/dataset/NEW_RAW_INCREASE_DATA/wholee7月13日-8月7日_漏检"
    ...
    for img in img_list:
        try:
            ...
            if (auto_result == auto_result_filter or auto_result_filter == None) \
                ...
                #print(img)
                if "textData" not in img : continue
                url = img["textData"]
                if final_result_filter=="Reject" and (auto_result_filter=="Accept" or auto_result_filter==None):
                    ...
                    if "finalTagHit" not in img: continue
                    final_brand_raw = list(set(img["finalTagHit"].split(",")))
                    final_brand_list = []
                    for i in final_brand_raw:
                        final_brand_list.append(i.split("/")[-1].split("】")[0])
                    if "ruleHit" not in img:
                        final_brand_raw_auto = []
                    else:
                    ...
                    if rr_match == 1:
                        ...
                        final_brand_raw = list(set(img["finalTagHit"].split(",")))
                elif auto_result_filter=="Reject" and ((final_result_filter=="Accept" and dict_brand["finalCheckResult"]=="Accept") or final_result_filter==None):
