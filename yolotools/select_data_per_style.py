...
def copy_file(xml, dst_dir,label):
    #anno_dir = os.path.join(dst_dir)
    image_dir = os.path.join(dst_dir)
    #stem = xml.stem
    ...
    img_p = os.path.join(image_dir, label.lower().replace(" ","_"))
    if not os.path.exists(img_p):
        os.makedirs(img_p)
    shutil.copyfile(img_path,os.path.join(img_p,img_path.name))
    # with open(str(img_path), 'rb') as fr_img:
    #     data = fr_img.read()
    #     img_p = os.path.join(image_dir, label)
    #     if not os.path.join(img_p):
    #         os.makedirs(img_p)
    #     fw_i = open(os.path.join(img_p,img_path.name), 'wb')
    #     fw_i.write(data)
    #     fw_i.close()
...
def select_data(styles_dict, anno_dir, dst_dir, min_num=20):
    ...
    for xml in tqdm(xmls):
        ...
        if iscopy:
            copy_file(xml, dst_dir,lab)
            if lab in styles_dict:
                styles_dict[lab] +=1
            else:
                styles_dict[lab] =1
