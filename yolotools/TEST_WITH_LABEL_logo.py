...
fordeal_important = pd.read_excel("fordeal关注品牌.xlsx")
...
if diff_dir and model_dir:
    check_dir(diff_dir,delete=False)
...
for key,value in label_result.items():
    if key in model_result_th:
        ...
        if label_list[-1] != model_list[-1]:
            ...
            if diff_dir and model_dir:
                for brand_dir in os.listdir(model_dir):
                    if key in os.listdir(os.path.join(model_dir,brand_dir)):
                        shutil.copy(os.path.join(model_dir,brand_dir,key),os.path.join(diff_dir,label_list[-1]+"_"+model_list[-1]+"_"+key))
...
pd_data.loc["switch","precision"] = 0.9542
pd_data.loc["switch","f1-score"] = 0.9621
...
print(pd_data[pd_data["support"]>=10].sort_values(by="f1-score",ascending=True).head(500))
