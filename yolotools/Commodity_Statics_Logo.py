...
#线上数据的txt文件
txt_dir = "/data01/xu.fx/dataset/NEW_RAW_INCREASE_DATA/3月18日_3月24日.txt"
#具体品牌表格输出目录
save_result_csv_dir = "./"
#check品牌配置，品牌：数量
check_brand = {"hermes":1000,"bvlgari":1000,"bottega_veneta":1000,"dragon_ball":1000}#None
#check品牌下载目录，未配置check品牌则无效
check_brand_img_save_dir = "/data01/xu.fx/dataset/NEW_RAW_INCREASE_DATA/tmp2"
#打印具体指标的出现次数要求，csv结果没有出现次数限制
total_support_th = 20
logo_support_th = 20
fordeal_important_support_th = 20
...
for line in tqdm(lines):
    ...
    if "finalTagHit" in dict_brand:
        if dict_brand["autoCheckResult"] == "Accept" and dict_brand["finalCheckResult"] == "Accept":
            ...
            human.append("accept")
            continue
        if "[图像]侵权/品牌" in dict_brand["finalTagHit"] or dict_brand["finalTagHit"] == "":#human get result will make sence
            list_per_pic = dict_brand["imageCommodityData"]
            auto_ = []
            for per_pic in list_per_pic:
                if check_brand:
                    human__ = []
                    if "finalTagHit" in per_pic and "[图像]侵权/品牌" in per_pic["finalTagHit"]:
                        for per_brand in per_pic["finalTagHit"].split(","):
                            if "[图像]侵权/品牌" in per_brand:
                                brand = per_brand.split("/")[-1]  # nike
                                human__.append(brand)
                if "ruleHit" in per_pic and "图像识别品牌" in per_pic["ruleHit"]:
                    for per_obj in per_pic["ruleHit"].split(","):  # 图像识别品牌【侵权/品牌/prada】
                        if "图像识别品牌" in per_obj:
