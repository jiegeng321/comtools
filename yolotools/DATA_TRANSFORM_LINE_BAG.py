...
main_data_dir = "dataset/BAG_DATASET/comb_data"
yolo_dataset_name = "yolodataset_bag_1bs_1ks_0819"
...
def get_brands_and_labels(src_dir):
    ...
    print(bcolors.OKGREEN + 'classes num list:'+ bcolors.ENDC, end=" ")
    for item in sorted(class_num, key=lambda a: class_num[a], reverse=True):
        print(bcolors.OKGREEN + "(%s" % item + ", " + "%d)" % class_num[item] + bcolors.ENDC, end=" ")
    print(bcolors.OKGREEN + "\nclasses num: "+ bcolors.ENDC, len(class_num))


    print('file head num list:' , end=" ")
    for item in sorted(file_num, key=lambda a: file_num[a], reverse=True):
        print("(%s" % item + ", " + "%d)" % file_num[item], end=" ")
    print("\nfile num: ", len(file_num))
