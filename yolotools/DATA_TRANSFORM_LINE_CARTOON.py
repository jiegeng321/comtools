...
white_sample_dir_list["/data01/xu.fx/dataset/CARTOON_DATASET/white_data/fordeal_white_data"] = 2500
# white_sample_dir_list["/data01/xu.fx/dataset/LOGO_DATASET/white_data/万维误检数据0705"] = 2500
# white_sample_dir_list["/data01/xu.fx/dataset/LOGO_DATASET/white_data/wholee误检数据0803"] = 1500
...
random_seed = 1
train_val_test_ratio = [1, 0.0, 0.0]
...
main_data_dir = "dataset/CARTOON_DATASET/comb_data"
yolo_dataset_name = "yolodataset_cartoon_55bs_57ks_0908"

CLASS_list = ['Pluto', 'Woody', 'Mike Wazowski', 'James P sullivan', 'Mater', 'Shaun the Sheep', 'Teenage Mutant Ninja Turtles', 'Bambi', 'hello Kitty', 'Jerry', 'Tom', 'Minions-2', 'Minions-1', 'Minions-3', 'Captain America', 'Peppa pig', 'super mario', 'Snoopy', 'Smurfs', 'Miffy', 'Dumbo', 'Deadpool', 'angry birds', 'Simba', 'Stitch', 'Iron Man', 'kaws COMPANION', 'Darth Vader', 'SpongeBob SquarePants', 'Black Panther', 'Buzz Lightyear', 'Patrick Star', 'Hulk', 'Spider Man', 'Tigger', 'Yoda', 'Squirtle', 'Pokemon Pikachu', 'Duck', 'Mouse', 'Goofy', '上帝十字架', '船锚', '猪元素', '犹太教六角形', '恶魔之眼', '南瓜灯', 'adidas_yeezy_350', '佛教配饰', '圣诞树', '圣诞老人', '法蒂玛之手', 'Lucas_the_spider', 'paw_patrol', 'adidas_yeezy_450']
D1 = ["UNICORN"]
D2 = ["圣诞帽子"]
CLASS_list = CLASS_list + D1 + D2
brand_names = []
for c in CLASS_list:
    brand = c.split("-")[0]
...
eval_img_path = "/data01/xu.fx/dataset/CARTOON_DATASET/fordeal_test_data/val/images"
eval_xml_path = "/data01/xu.fx/dataset/CARTOON_DATASET/fordeal_test_data/val/labels_xml"
...
def dataset_check(yolo_dataset_dir):
    ...
    val_img = yolo_dataset_dir+'/'+'JPEGImages/val/images'
    ...
    label_val = yolo_dataset_dir + '/' + 'JPEGImages/val/labels'
    ...
    print("val img and label num: ", val_img_num,label_val_num,"with white sample: ",val_img_white_num)
    ...
    if train_img_num==label_train_num and val_img_num==label_val_num and test_img_num==label_test_num:
    ...
    for anno_file in tqdm(xml_ps):
        ...
        for obj in root.iter('object'):
            ...
            if cls is None:
                continue
            cls = cls.strip()
            if cls in ['Daisy Duck', 'Donald Duck']:
                cls = "Duck"
            elif cls in ['micky minnie', 'Micky mouse', 'Minnie Mouse']:
                cls = "Mouse"
            elif cls in ["UNICORN-hard", "UNICORN"]:
                cls = "UNICORN"
...
def split_data_and_rename_func(src_dir,brand_files,class_list_sts,class_list_sts_total,file_num,file_num_total):
    ...
    for i, files in enumerate(brand_files):
        ...
        for index, file_i in tqdm(enumerate(files)):
            ...
            img_data = fimg.read()
            fimg.close()
            val_txt_path = ospathjoin([yolo_dataset_dir, 'JPEGImages/val/labels',xml_file.parent.name.replace(' ', '').replace("'", "").replace('&','') + "_" +xml_file.name.replace('-', '').replace(' ', '').replace('&', '').replace(".xml",".txt")])
            if index < val_num:
                if index<detect_num and detect_img_dir!=None:
                    ...
                    with open(detect_img_path, 'wb') as fw:
                        fw.write(img_data)
                dst_img_path = ospathjoin([yolo_dataset_dir, 'JPEGImages/val/images',
                                           xml_file.parent.name.replace(' ', '').replace("'", "").replace('&', '') + "_" +
                                           img_path.split('/')[-1].replace('-', '').replace(' ', '').replace('&', '')])
                with open(dst_img_path, 'wb') as fw:
                    fw.write(img_data)
                xml2txt(val_txt_path,cls_boxes,size,class_list_sts)
                ...
                with open(dst_img_path, 'wb') as fw:
                    fw.write(img_data)
                test_txt_path = val_txt_path.replace('val', 'test')
                ...
                with open(dst_img_path, 'wb') as fw:
                    fw.write(img_data)
                train_txt_path = val_txt_path.replace('val', 'train')
                xml2txt(train_txt_path, cls_boxes, size, class_list_sts)
def split_data_and_rename_eval(imgs_path,xmls_path):
    if detect_img_dir:
    ...
    check_dir(yolo_dataset_dir_path / "JPEGImages/val/images")
    check_dir(yolo_dataset_dir_path / "JPEGImages/test/images")
    check_dir(yolo_dataset_dir_path / "JPEGImages/train/labels")
    check_dir(yolo_dataset_dir_path / "JPEGImages/val/labels")
    check_dir(yolo_dataset_dir_path / "JPEGImages/test/labels")
    # xml_files = Path(xmls_path).rglob("*.xml")
    img_files = [i for i in Path(imgs_path).rglob("*.*") if is_img(i)]
    for index, img_file in tqdm(enumerate(img_files)):
        xml_file = Path(xmls_path) / img_file.with_suffix(".xml").name
        if xml_file.exists():
            objs,size = get_xml_obj_num(xml_file)
            if objs == -1:
                continue
            ...
            for obj in objs:
                ...
                cls_boxes.append([cls, float(xmin), float(xmax), float(ymin), float(ymax)])
            # img_path = None
            # for fix in [".jpg",".jpeg",".JPG",".JPEG",".png",".PNG"]:
            #     if (Path(imgs_path) / xml_file.with_suffix(fix).name).exists():
            #         img_path = Path(imgs_path) / xml_file.with_suffix(fix).name
            #         break
            # if not img_path:
            #     print('Image File Not Found')
            #     continue
            shutil.copy(str(img_file),os.path.join(yolo_dataset_dir, 'JPEGImages/val/images',img_file.name.replace(' ', '').replace('&','')))
            val_txt_path = ospathjoin([yolo_dataset_dir, 'JPEGImages/val/labels', xml_file.name.replace(".xml",".txt").replace(' ', '').replace('&','')])
            xml2txt(val_txt_path, cls_boxes, size, [])
        else:
            shutil.copy(str(img_file), os.path.join(yolo_dataset_dir, 'JPEGImages/val/images',
                                                    img_file.name.replace(' ', '').replace('&', '')))
            val_txt_path = ospathjoin([yolo_dataset_dir, 'JPEGImages/val/labels',
                                       xml_file.name.replace(".xml", ".txt").replace(' ', '').replace(
                                           '&', '')])
...
#     val_label_path = train_label_path.replace('train', 'val')
...
def add_white_sample_func(yolo_dataset_dir,white_sample_list):
    ...
    val_white_num = int(white_sample_num * train_val_test_ratio[1])
    test_white_num = white_sample_num - train_white_num - val_white_num
    for i in tqdm(range(white_sample_num)):
        ...
        if i < test_white_num:
            ...
            with open(os.path.join(label_test, "checked_WhiteSample_" + white_sample_list[i].name[:-(
                ...
                pass
        elif i < test_white_num + val_white_num:
            shutil.copy(white_sample_list[i],
                        os.path.join(val_img, "checked_WhiteSample_" + white_sample_list[i].name))
            with open(os.path.join(label_val, "checked_WhiteSample_" + white_sample_list[i].name[:-(
                    len(white_sample_list[i].name.split('.')[-1]))] + 'txt'), 'w') as f:
                pass
        else:
            shutil.copy(white_sample_list[i],
...
if show_data_info:
    ...
    print("-" * 20 + "copy val start" + "-" * 20)
    split_data_and_rename_eval(eval_img_path, eval_xml_path)
    print("-" * 20 + "copy val end" + "-" * 20)
