...
src_dir = Path("/data01/xu.fx/dataset/LOGO_DATASET/fordeal_test_data_total/brand_labeled")
min_size = 10
hashs = "totalhash"#ahash,dhash,dhash,totalhash
hash_th = 40
hash_size = (16, 16)
WORKERS = 10
...
if __name__ == "__main__":
    # brands = {}
    # for file in files:
    #     if file.name.split("_")[0] in brands:
    #         brands[file.name.split("_")[0]].append(file)
    #     else:
    #         brands[file.name.split("_")[0]] = [file]
    # brands_len = len(brands)
    # #print(len(brands))
    # i=1
    # for k,v in brands.items():
    #     print(k,":",i,"/",brands_len)
    #     same_img_mv(v)
    #     i += 1
    # for i in tqdm(os.listdir(src_dir)):
    #     if i == "empty":
    #         continue
    #     file = Path(os.path.join(src_dir,i))
    #     files = sorted([p for p in file.rglob("*.*") if is_img(p)])
    #     same_img_mv(files)
    bad_img_mv(files)
