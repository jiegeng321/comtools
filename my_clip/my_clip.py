...
brand_total = ["allsaints","american_eagle","hamilton","jack&jones","reebok","maybelline","carolina_herrera","aspinal_of_london","iron_maiden","popsockets"]

# if "kappa" not in brand_total:
#     brand_total.append("kappa")
#brand_total = ["kappa",'3m', '3t', '511 tactical', '7up']
for i in range(len(brand_total)):
    brand_total[i] = "a photo of " + brand_total[i].replace("_"," ") + " brand"
    #brand_total[i] += " brand"
brand_total_np = np.array(brand_total)
...
def clip_func2(img,top_n,soft_max=False):
    ...
    with torch.no_grad():
        ...
        for logo, score in zip(brand_total_np[indices], cos_sim[0][indices]):
            logo = logo.replace("a photo of ", "").replace(" brand", "")
            logo = logo.replace(" ", "_").replace("&","_")
            res.append({"logo_name": logo, "score": score})
        return {"res": res}
if __name__=="__main__":
    img = "/data01/xu.fx/dataset/LOGO_DATASET/fordeal_test_data/brand_labeled/ecko/checked_ecko_ECKO150.jpg"
    img_dir = "/data01/xu.fx/dataset/LOGO_DATASET/fordeal_test_data/brand_labeled/nintendo/"
    print(clip.available_models())
    # res = clip_func2(img, 5,soft_max=True)
    # res2 = clip_func(img, 5)
    # print(res)
    # print(res2)

    # for k,i in enumerate(Path(img_dir).rglob("*.*")):
    #     res = clip_func2(str(i),5)
